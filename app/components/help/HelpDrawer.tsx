...
export default function HelpDrawer({
  open, onClose, sections, examples, currentExampleId, onLoadExample,
}: Readonly<HelpDrawerProps>) {
  ...
  const relatedExample = active?.relatedExample
    ? examples.find(e => e.id === active.relatedExample)
    : undefined
  const relatedExampleLoaded = !!relatedExample && relatedExample.id === currentExampleId
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 min-w-0" tabIndex={0}>
            {active ? (
              <>
                {relatedExample && (
                  <div className="flex items-center justify-between gap-3 mb-4 px-3 py-2.5 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                    <span className="text-xs text-blue-200">
                      Ejemplo relacionado: <strong className="font-semibold">{relatedExample.label}</strong>
                    </span>
                    <button
                      type="button"
                      disabled={relatedExampleLoaded}
                      onClick={() => onLoadExample(relatedExample)}
                      className={[
                        'shrink-0 px-3 py-1.5 rounded text-xs font-medium transition-colors',
                        relatedExampleLoaded
                          ? 'bg-zinc-700/50 text-zinc-400 cursor-default'
                          : 'bg-blue-600 text-white hover:bg-blue-500',
                      ].join(' ')}
                    >
                      {relatedExampleLoaded ? 'Ejemplo cargado' : 'Cargar ejemplo'}
                    </button>
                  </div>
                )}
