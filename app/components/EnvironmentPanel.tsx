...
function cardHeight(frame: EnvFrame): number {
  const scopes = frame.scopes ?? [frame.bindings]
  const bindingRows = frame.bindings.length
  const separators = Math.max(0, scopes.length - 1)
  return CARD_H_HEADER + CARD_PAD_V + bindingRows * CARD_H_ROW + separators * CARD_H_SEP + CARD_PAD_V
}
...
function FrameCard({ frame, x, y }: { frame: EnvFrame; x: number; y: number }) {
...
export default function EnvironmentPanel({ frames }: Readonly<EnvironmentPanelProps>) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#1e1e1e]">
      <Header count={frames.length} onReset={reset} />

      {/* El ref siempre está montado para que usePanZoom pueda attachear listeners */}
      <div
        ref={containerRef}
...
function Header({ count, onReset }: { count: number; onReset: () => void }) {
  return (
