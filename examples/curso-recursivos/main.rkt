...
;;; main.rkt: evaluador con let, proc, aplicación, letrec y primitivas.
...
(define evaluar-expresion
  (lambda (exp amb)
    (cases expresion exp
      ...
      ;; Booleanos como valores expresados.
      (true-exp () #true)
      (false-exp () #false)
      ;; Primitiva: los argumentos se evalúan antes de operar.
      (prim-exp (prim args)
        (let ([lista-numeros
               (map (lambda (x) (evaluar-expresion x amb)) args)])
          (evaluar-primitiva prim lista-numeros)))
      ;; Condicional: la condición debe ser booleana y se evalúa una rama.
      ...
      ;; Ligadura local: los valores se calculan en el ambiente de afuera.
      ...
      ;; Procedimiento: la clausura captura el ambiente de creación.
      (proc-exp (ids body)
        (closure ids body amb))
      ;; Aplicación: verifica que sea procedimiento y la aridad.
      (app-exp (rator rands)
              ...
              (cases procval procV
                (closure (lid body old-env)
                  (if (= (length lid) (length lrands))
                      ...
                      (eopl:error
                       "El número de argumentos no es correcto, debe enviar"
                       (length lid) " y usted ha enviado" (length lrands)))))
              (eopl:error
               "No puede evaluarse algo que no sea un procedimiento"
               procV))))
      ;; letrec: el cuerpo se evalúa en el marco recursivo, que permite la
      ;; recursión y la recursión mutua (ver environment.rkt).
